...
| R-03 | Uszkodzone lub niespójne dane PKW (brak okręgu, NaN, ujemne, suma głosów ≠ ważne, brak partii)                                                            | Cicha degradacja rozkładu geograficznego; symulacja „działa", ale na złych danych    | niskie (dane generowane skryptem), wysoki skutek | Testy integralności dla każdego pliku `src/data/*.json` (w tym `parlamentarne-2011.json` pobrany skryptem ze strony PKW): 41 okręgów, metadane, brak NaN, suma głosów ≈ ważne; każda dystrybucja z `partyDefaults` (np. proxy PJN 2011 dla Rozwój+) wskazuje na istniejące wybory i partię | `src/data/__tests__/data-integrity.test.ts`, `src/data/__tests__/party-mapping.test.ts`                                                                                                      | covered  |
...
| `src/data/__tests__/data-integrity.test.ts`        | R-03                     | 6 per zbiór wyborów (5 zbiorów, w tym 2011) |
| `src/data/__tests__/party-mapping.test.ts`         | R-03                     | 1 per dystrybucja w `partyDefaults` + 1     |
